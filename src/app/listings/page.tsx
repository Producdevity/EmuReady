...
import { DevicePhoneMobileIcon, CpuChipIcon, RocketLaunchIcon, MagnifyingGlassIcon, AdjustmentsHorizontalIcon, EyeIcon, PencilSquareIcon, TrashIcon } from "@heroicons/react/24/outline";
...
function SuccessRateBar({ rate }: { rate: number }) {
  ...
  return (
    <div className="w-full h-4 bg-gray-200 dark:bg-gray-700 rounded">
      ...
      />
      <span className="ml-2 text-xs text-gray-700 dark:text-gray-200 align-middle">{rate}%</span>
    </div>
  );
}
...
export default function ListingsPage() {
  ...
  const [editGame, setEditGame] = useState<{ id: string; title: string; systemId: string } | null>(null);
  ...
  const handleDeviceChange = (e: React.ChangeEvent<HTMLSelectElement>) => setDeviceId(e.target.value);
  const handleEmulatorChange = (e: React.ChangeEvent<HTMLSelectElement>) => setEmulatorId(e.target.value);
  const handlePerformanceChange = (e: React.ChangeEvent<HTMLSelectElement>) => setPerformanceId(e.target.value);

  if (isLoading) return <div className="p-8 text-center">Loading games...</div>;
  if (error) return <div className="p-8 text-center text-red-500">Failed to load games.</div>;
  ...
  return (
    <main className="flex flex-col md:flex-row min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <aside className="w-full md:w-64 bg-white dark:bg-gray-800 p-4 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 flex-shrink-0 rounded-2xl shadow-xl">
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2"><AdjustmentsHorizontalIcon className="w-5 h-5 text-blue-500" /> Filters</h2>
        <div className="space-y-4">
          <div>
            <label className="block mb-1 font-medium">System</label>
            <select className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 transition-all" value={systemId} onChange={handleFilterChange} disabled={systemsLoading}>
          ...
          <div>
            ...
            >
              ...
              {devices?.map((dev: { id: string; brand: string; modelName: string }) => (
                <option key={dev.id} value={dev.id}>{dev.brand} {dev.modelName}</option>
              ))}
            </Input>
          </div>
          <div>
          ...
          <div>
            ...
            >
              ...
              {performanceScales?.map((perf: { id: number; label: string }) => (
                <option key={perf.id} value={perf.id}>{perf.label}</option>
              ))}
            </Input>
          </div>
          <div>
      ...
      <section className="flex-1 p-4 overflow-x-auto">
        <h1 className="text-3xl font-extrabold mb-8 text-gray-900 dark:text-white tracking-tight">Game Listings</h1>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800 rounded-2xl">
            <thead className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-800 dark:via-gray-900 dark:to-gray-800">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Success Rate</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Listings</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {games.length === 0 && (
                <tr><td colSpan={6} className="text-center text-gray-500 py-8">No games found.</td></tr>
              )}
              {games.map((game: { id: string; title: string; system?: { id: string; name: string }; systemId: string; _count?: { listings: number }, listings?: (Listing & { performance?: PerformanceScale })[] }) => {
                // Calculate success rate (e.g., % of listings with performance 'Perfect' or 'Great')
                const total = game.listings?.length ?? 0;
                const good = game.listings?.filter(l => l.performance?.label === 'Perfect' || l.performance?.label === 'Great').length ?? 0;
                ...
                game.listings?.forEach(l => { if (l.performance?.label) perfCounts[l.performance.label] = (perfCounts[l.performance.label] || 0) + 1; });
                const topPerf = Object.entries(perfCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? '';
                let badgeVariant: 'secondary' | 'success' | 'info' | 'warning' | 'danger' = 'secondary';
                ...
                return (
                  <tr key={game.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-4 py-2 font-medium text-gray-900 dark:text-gray-100">{game.title}</td>
                    <td className="px-4 py-2 text-gray-700 dark:text-gray-300">{game.system?.name}</td>
                    <td className="px-4 py-2"><Badge variant={badgeVariant}>{topPerf || 'N/A'}</Badge></td>
                    <td className="px-4 py-2"><SuccessRateBar rate={successRate} /></td>
                    <td className="px-4 py-2 text-center">{game._count?.listings ?? 0}</td>
                    <td className="px-4 py-2 flex gap-2">
                      <button aria-label="View Listings" className="flex items-center gap-1 px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-150 shadow-sm hover:scale-105 focus:ring-2 focus:ring-blue-400" onClick={() => { setSelectedGame({ id: game.id, title: game.title }); setListingsModalOpen(true); }}>
                        <EyeIcon className="w-5 h-5" /> View
                      </button>
                      {session?.user.role === "ADMIN" && (
                        <>
                          <button aria-label="Edit Game" className="flex items-center gap-1 px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-all duration-150 shadow-sm hover:scale-105 focus:ring-2 focus:ring-yellow-400" onClick={() => { setEditGame({ id: game.id, title: game.title, systemId: game.systemId }); setEditTitle(game.title); setEditSystemId(game.systemId); setEditModalOpen(true); }}>
                            <PencilSquareIcon className="w-5 h-5" /> Edit
                          </button>
                          <button aria-label="Delete Game" className="flex items-center gap-1 px-3 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-150 shadow-sm hover:scale-105 focus:ring-2 focus:ring-red-400" onClick={async () => { if (confirm(`Delete game \"${game.title}\"? This cannot be undone.`)) { await deleteGame.mutateAsync({ id: game.id }); } }} disabled={deleteGame.isPending}>
                            <TrashIcon className="w-5 h-5" /> {deleteGame.isPending ? "Deleting..." : "Delete"}
                          </button>
                        </>
                      )}
                    </td>
              ...
              {Array.from({ length: pagination.pages }, (_, i) => i + 1)
                ...
                .map((p, idx) =>
                  typeof p === 'number' ? (
                    <li key={p}>
                      <button
                        className={`px-4 py-2 rounded-md border text-sm font-medium transition ${p === page
                    ...
                    <li key={`ellipsis-${idx}`} className="flex items-center px-2 text-gray-400">…</li>
                  )
                )}
              {/* Next button */}
              <li>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 w-full max-w-md shadow-lg relative">
              ...
              <form
                onSubmit={async e => {
                  e.preventDefault();
                  await updateGame.mutateAsync({ id: editGame.id, title: editTitle, systemId: editSystemId });
        ...
        {session?.user && (
          <Link href="/listings/new">
            <button className="mt-10 px-8 py-4 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white rounded-xl font-bold text-lg shadow-lg transition-all duration-200 transform hover:scale-105 w-full max-w-xs mx-auto block">Add Listing</button>
          </Link>
        )}
      </section>
    </main>
...
function GameListingsModal({ gameId, gameTitle, onClose, session }: { gameId: string; gameTitle: string; onClose: () => void; session: Session | null }) {
  const { data, isLoading, error } = api.listings.list.useQuery({ systemId: gameId });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 rounded-lg p-6 w-full max-w-2xl shadow-lg relative">
        ...
        <div className="space-y-4 max-h-96 overflow-y-auto">
          {listings.length === 0 && <div className="text-gray-500">No listings yet.</div>}
          {listings.map((listing: { id: string; device?: { brand: string; modelName: string }; emulator?: { name: string }; performance?: { label: string }; author?: { name: string | null } }) => (
            <div key={listing.id} className="border border-gray-300 dark:border-gray-700 rounded p-4">
              <div className="font-semibold">Device: {listing.device?.brand} {listing.device?.modelName}</div>
        ...
        {session?.user && (
          <button className="mt-6 px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700" onClick={() => alert('Add listing not implemented yet')}>Add Listing</button>
        )}
      </div>
    </div>
  );
