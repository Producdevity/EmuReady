...
function Autocomplete<T extends AutocompleteOptionBase>({
  ...
  useEffect(() => {
    ...
    const selectedItem = allAvailableItems.find(item => optionToValue(item) === value);
    if (selectedItem) {
  ...
  }, [value, staticItems, optionToValue, optionToLabel, suggestions, loadItems, inputValue]);
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (!isOpen && e.key !== 'Tab') {
        ...
        if (inputValue.length >= minCharsToTrigger && !loadItems) { // for static items, re-filter
  ...
  const handleInputFocus = () => {
    ...
    if (inputValue.length >= minCharsToTrigger || (!loadItems && staticItems && staticItems.length > 0 && inputValue.length === 0) ) {
  ...
  const showNoResults = isOpen && !isLoading && suggestions.length === 0 && inputValue.length >= minCharsToTrigger;
  const showMinCharsMessage = isOpen && !isLoading && inputValue.length < minCharsToTrigger && (!staticItems || staticItems.length === 0 || loadItems);
  ...
  return (
    <div className={twMerge('relative', className)}>
      ...
      {isOpen && (suggestions.length > 0 || showNoResults || showMinCharsMessage) && (
         <ul
          ref={listRef}
        ...
        >
          ...
          {!isLoading && showNoResults && (
            <li className="px-4 py-2 text-gray-500 dark:text-gray-400 text-center">No results found.</li>
          )}
          ...
          {!isLoading && suggestions.map((item, idx) => {
            ...
            return (
              <li
                ...
                onMouseDown={({ preventDefault }) => { preventDefault(); handleOptionClick(item); }}
                onMouseEnter={() => setHighlightedIndex(idx)}
              >
                {itemIcon && <span className="mr-2 flex-shrink-0">{itemIcon}</span>}
