...
export const mobilePreferencesRouter = createMobileTRPCRouter({
  ...
  /**
   * @deprecated Use 'addDevice' instead
   */
  ...
  /**
   * @deprecated Use 'bulkUpdateDevices' instead
   */
  ...
  /**
   * @deprecated Use 'bulkUpdateSocs' instead
   */
  ...
  /**
   * @deprecated Use 'currentProfile' instead
   */
  getCurrentUserProfile: mobileProtectedProcedure.query(async ({ ctx }) => {
    return await ctx.prisma.user.findUnique({
      where: { id: ctx.session.user.id },
      select: {
        id: true,
