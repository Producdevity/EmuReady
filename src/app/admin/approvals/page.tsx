...
type PendingListing = inferProcedureOutput<AppRouter['listings']['listPending']>[number];

export default function AdminApprovalsPage() {
  const { data: pendingListings, isLoading, refetch } = api.listings.listPending.useQuery(undefined, {
  ...
  const [selectedListingForRejection, setSelectedListingForRejection] = useState<PendingListing | null>(null);
  ...
  if (isLoading) {
    return <div className="container mx-auto p-6"><p>Loading pending approvals...</p></div>;
  }

  if (!pendingListings || pendingListings.length === 0) {
    return (
      <div className="container mx-auto p-6">
        <h1 className="text-3xl font-bold mb-8 text-gray-800 dark:text-white">Listing Approvals</h1>
        <p className="text-gray-600 dark:text-gray-400">No listings are currently awaiting approval.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">System</th>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Emulator</th>
            ...
            {pendingListings.map((listing) => (
              <tr key={listing.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                  <Link href={`/listings/${listing.id}`} target="_blank" className="hover:text-blue-600 dark:hover:text-blue-400 flex items-center">
                    {listing.game.title}
                    <ExternalLink className="ml-1.5 h-3 w-3 text-gray-400 dark:text-gray-500" />
                  </Link>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{listing.game.system.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                  {listing.device.brand.name} {listing.device.modelName}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{listing.emulator.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                  {listing.author?.name ?? 'N/A'} 
                  <span className="text-xs text-gray-400 dark:text-gray-500">({listing.author?.email ?? 'No email'})</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400" title={new Date(listing.createdAt).toLocaleString()}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2 flex items-center">
                  <Button 
                    ...
                    isLoading={approveMutation.isPending && approveMutation.variables?.listingId === listing.id}
                    aria-label="Approve"
      ...
      {showRejectionModal && selectedListingForRejection && (
        <Modal isOpen={showRejectionModal} onClose={closeRejectionModal} title={`Reject Listing: ${selectedListingForRejection.game.title}`}>
          <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-300">
              You are about to reject the listing for 
              <strong className="text-gray-800 dark:text-white"> {selectedListingForRejection.game.title}</strong> on 
              <strong className="text-gray-800 dark:text-white"> {selectedListingForRejection.device.brand.name} {selectedListingForRejection.device.modelName}</strong> (submitted by {selectedListingForRejection.author?.name ?? 'Unknown User'}).
            </p>
            <div>
              <label htmlFor="rejectionNotes" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t dark:border-gray-700 mt-6">
              <Button variant="outline" onClick={closeRejectionModal} disabled={rejectMutation.isPending}>Cancel</Button>
              <Button 
                variant="danger" 
