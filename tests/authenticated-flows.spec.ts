  ...
  test('should access profile settings after authentication', async ({ page }) => {
    ...
    if (isAuthenticated) {
      ...
      const profileSections = {
        ...
        'Notification Settings': '[data-testid*="notification"], .notification-settings',
      ...
      // Profile actions
      const actions = ['Edit Profile', 'Change Password', 'Manage Devices', 'Download Data']
    ...
    if (await authPage.isAuthenticated()) {
      ...
      if (await notificationList.isVisible({ timeout: 3000 })) {
        ...
        if (notifCount > 0) {
          ...
          // Mark as read action
          const markRead = firstNotif.locator('button').filter({ hasText: /mark.*read/i })
        ...
        const markAllRead = page.locator('button').filter({ hasText: /mark.*all.*read/i })
    ...
    if (await authPage.isAuthenticated()) {
      ...
      if (await listingsTab.isVisible({ timeout: 3000 })) {
        ...
        if (listingCount > 0) {
          ...
          // Delete button
          const deleteButton = firstListing.locator('button').filter({ hasText: /delete/i })
  ...
  test('should participate in community features', async ({ page }) => {
    ...
    if (await authPage.isAuthenticated()) {
      ...
      if ((await listingsPage.getListingCount()) > 0) {
        ...
        const reportButton = page.locator('button, a').filter({ hasText: /report/i })
  ...
  test('should show personalized content', async ({ page }) => {
    ...
    if (await authPage.isAuthenticated()) {
      ...
      const personalizedSections = {
        ...
        'Continue Where You Left Off': '[data-testid*="continue"], .recent-views',
      }
      ...
      const myDevicesFilter = page.locator('button, label').filter({ hasText: /my.*devices/i })
