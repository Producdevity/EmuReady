...
function VoteReminderBanner(props: Props) {
  ...
  // Hide banner when user votes elsewhere after it's been shown
  useEffect(() => {
    if (isVisible && props.currentVote !== null) {
      setIsVisible(false)
    }
  }, [props.currentVote, isVisible])
  ...
  // Don't render component at all if dismissed, no message, or user already voted on mount
  if (hasBeenDismissed || !selectedMessage || props.currentVote !== null) {
    return null
  }
