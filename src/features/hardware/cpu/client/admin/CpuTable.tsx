...
interface Props {
  ...
  onView: (cpu: CpuDetail) => void
  onEdit: (cpu: CpuDetail) => void
  onDelete: (id: string) => void
}

export function CpuTable(props: Props) {
  if (props.cpus.length === 0) {
    return <AdminTableNoResults icon={Cpu} hasQuery={props.hasQuery} />
  }
  ...
  return (
    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
      ...
      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
        {props.cpus.map((cpu) => (
          <tr key={cpu.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
            {props.columnVisibility.isColumnVisible('brand') && (
            ...
            {props.columnVisibility.isColumnVisible('listings') && (
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                <Badge>{cpu.pcListingCount}</Badge>
              </td>
            )}
            {props.columnVisibility.isColumnVisible('actions') && (
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div className="flex items-center gap-2">
                  <ViewButton onClick={() => props.onView(cpu)} title="View CPU Details" />
                  {props.canManageDevices && (
                    <EditButton onClick={() => props.onEdit(cpu)} title="Edit CPU" />
                  )}
                  {props.canManageDevices && (
                    <DeleteButton
                      onClick={() => props.onDelete(cpu.id)}
                      title="Delete CPU"
