...
export function Pagination(props: Props) {
  ...
  const showNavigation = props.totalPages > 1
  ...
  const navigationButtons = showNavigation && (
    <div className="flex items-center space-x-1">
      <button
        ...
        className={cn(
          'relative inline-flex items-center px-2 sm:px-3 py-1.5 sm:py-2 text-sm font-medium rounded-lg transition-all duration-200',
          props.page === 1
      ...
      >
        ...
        <span className="hidden lg:ml-1 lg:inline">Prev</span>
      </button>
      ...
      {props.page > 3 && (
        <>
          <button
            ...
            className="relative hidden sm:inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 shadow-sm hover:shadow-md transition-all duration-200"
          ...
          {props.page > 4 && (
            <span
              className="hidden sm:flex items-center px-1 py-2 text-gray-500 dark:text-gray-400"
              aria-hidden="true"
            >
      ...
      {pagesAroundCurrent.map((pageNum) => (
        <button
          ...
          className={cn(
            'relative inline-flex items-center px-2.5 sm:px-3 py-1.5 sm:py-2 text-sm font-medium rounded-lg transition-all duration-200',
            pageNum === props.page
      ...
      >
        <span className="hidden lg:mr-1 lg:inline">Next</span>
        <ChevronRight className="w-4 h-4" />
      </button>
    </div>
  )

  const showingLabel = showLabel && props.totalItems && (
    <div className="text-sm text-gray-600 dark:text-gray-400 font-medium whitespace-nowrap">
      <span className="hidden sm:inline">
        Showing <span className="font-semibold text-gray-900 dark:text-gray-100">{itemsStart}</span>{' '}
        to <span className="font-semibold text-gray-900 dark:text-gray-100">{itemsEnd}</span> of{' '}
        <span className="font-semibold text-gray-900 dark:text-gray-100">{props.totalItems}</span>{' '}
        results
      </span>
  ...
  if (!showNavigation && !props.pageSizeSelector && !showingLabel) {
    return null
  }
  ...
  return (
    <nav className="mt-4" aria-label="Pagination navigation" role="navigation">
      ...
      <div className="hidden md:flex items-center justify-between gap-4">
        <div className="flex-shrink-0">{props.pageSizeSelector}</div>
        <div className="flex-1 flex justify-center">{showingLabel}</div>
        <div className="flex-shrink-0">{navigationButtons}</div>
      </div>

      {/* Mobile: Stacked layout */}
      <div className="md:hidden flex flex-col items-center gap-3">
        {props.pageSizeSelector && <div>{props.pageSizeSelector}</div>}
        {showingLabel}
        {navigationButtons}
        {showNavigation && (
