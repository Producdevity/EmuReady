...
async function makeRequest<T>(endpoint: string, params: Record<string, string> = {}): Promise<T> {
  ...
  try {
    ...
    throw new RawgError(
      `Failed to fetch from RAWG API: ${error instanceof Error ? error.message : 'Unknown error'}`,
      undefined,
      endpoint
    )
  }
...
function scoreGameMatch(gameName: string, searchQuery: string): number {
  ...
  if (normalizedGameName === normalizedQuery) {
    return 1000
  }
  ...
  if (matchingWords.length === queryWords.length) {
    ...
    for (const queryWord of queryWords) {
      const foundIndex = gameWords.findIndex((gameWord, index) => 
        index > lastIndex && gameWord.includes(queryWord)
      )
...
export async function searchGames(query: string, page = 1, pageSize = 10): Promise<RawgGameResponse> {
  if (!query.trim()) {
...
export async function getGameScreenshots(gameId: number, page = 1, pageSize = 20): Promise<RawgScreenshotsResponse> {
...
export async function searchGameImages(query: string): Promise<Map<number, GameImageOption[]>> {
  try {
    ...
    throw new RawgError(`Failed to search for game images: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
