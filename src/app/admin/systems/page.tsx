        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800 rounded-2xl">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {systems?.map((sys: { id: string; name: string }) => (
              <tr key={sys.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-4 py-2">{sys.name}</td>
                <td className="px-4 py-2 flex gap-2 justify-end">
                  <Button variant="secondary" onClick={() => openModal(sys)}>Edit</Button>
                  <Button variant="danger" onClick={() => handleDelete(sys.id)}>Delete</Button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 transition-all">
          <form className="bg-white dark:bg-gray-900 rounded-2xl p-8 w-full max-w-md shadow-2xl border border-gray-200 dark:border-gray-800 relative" onSubmit={handleSubmit}>
            <h2 className="text-2xl font-extrabold mb-6 text-gray-900 dark:text-white tracking-tight">{editId ? "Edit System" : "Add System"}</h2>
