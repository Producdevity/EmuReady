...
const mockGame = {
  ...
  imageUrl: null,
  createdAt: new Date(),
  listings: [], // Add empty listings array
}
...
describe('Games Router - Approval System', () => {
  ...
  describe('delete game (admin only)', () => {
    it('should allow admins to delete games', async () => {
      // Setup mocks
      vi.mocked(prisma.game.findUnique).mockResolvedValue({
        ...mockGame,
        system: mockSystem,
        listings: [], // Empty listings array so deletion is allowed
      } as any)
      ...
      await adminCaller.games.delete({
        id: mockGameId,
      })
    ...
    it('should prevent non-admin users from deleting games', async () => {
      const ctx = createInnerTRPCContext({
        session: { user: mockUser },
      })
      const userCaller = appRouter.createCaller(ctx)
  ...
  describe('edit game (admin only)', () => {
    it('should allow admins to edit any game', async () => {
      // Setup mocks
      vi.mocked(prisma.game.findUnique).mockResolvedValue({
        ...mockGame,
        system: mockSystem,
      ...
      const result = await adminCaller.games.update({
        id: mockGameId,
        title: 'Admin Updated Game',
        systemId: mockSystemId,
      })

      expect(result.title).toBe('Admin Updated Game')
      expect(prisma.game.update).toHaveBeenCalledWith({
        where: { id: mockGameId },
        data: {
          title: 'Admin Updated Game',
    ...
    it('should prevent non-admin users from editing games', async () => {
      const ctx = createInnerTRPCContext({
        session: { user: mockUser },
      })
      const userCaller = appRouter.createCaller(ctx)
      ...
      await expect(
        userCaller.games.update({
          id: mockGameId,
          title: 'User Updated Game',
          systemId: mockSystemId,
        }),
      ).rejects.toThrow(TRPCError)
    })
  ...
  describe('approveGame (admin only)', () => {
    it('should allow admins to approve pending games', async () => {
      // Setup mocks
  ...
  describe('getGameStats (admin only)', () => {
    it('should return game statistics for admins', async () => {
      // Setup mocks
    ...
    it('should prevent non-admin users from accessing game stats', async () => {
      const ctx = createInnerTRPCContext({ session: { user: mockUser } })
      const userCaller = appRouter.createCaller(ctx)
