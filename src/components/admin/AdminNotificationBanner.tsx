import { AlertCircle, Clock, CheckCircle, XCircle, Bell, type LucideIcon } from 'lucide-react'
...
export function AdminNotificationBanner(props: Props) {
  const variant = variantStyles[props.type] ? variantStyles[props.type] : variantStyles.default

  const Icon = variant.icon

  return (
    <div className={cn('p-4 border rounded-lg mb-4', variant.containerClass, props.className)}>
      <div className="flex items-start">
        <Icon className={cn('w-5 h-5 mr-3 mt-0.5', variant.iconClass)} />
        <div className="flex-1">
          <h3 className={cn('text-sm font-medium', variant.textClass)}>{props.title}</h3>
          <p className={cn('mt-1 text-sm', variant.textClass)}>{props.message}</p>
