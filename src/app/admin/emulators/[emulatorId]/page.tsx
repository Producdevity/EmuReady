...
export default function EditEmulatorPage() {
  ...
  const { data: allSystems, isLoading: isLoadingSystems, error: systemsError } = api.systems.list.useQuery({});

  if (isLoadingEmulator || isLoadingSystems) {
    return <div className="container mx-auto p-4"><p>Loading emulator and systems data...</p></div>;
  }
    ...
    return (
      <div className="container mx-auto p-4">
        <p className="text-red-500">
          Error loading data: {emulatorError?.message ?? systemsError?.message ?? 'An unknown error occurred'}
        </p>
      </div>
    );
  }
  ...
  if (!emulator) {
    return <div className="container mx-auto p-4"><p>Emulator not found.</p></div>;
  }

  if (!allSystems) {
    return <div className="container mx-auto p-4"><p>No systems found to configure.</p></div>;
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      ...
      <h1 className="text-3xl font-bold mb-8 text-gray-800 dark:text-white">
        Edit Emulator: <span className="text-blue-600 dark:text-blue-400">{emulator.name}</span>
      </h1>
