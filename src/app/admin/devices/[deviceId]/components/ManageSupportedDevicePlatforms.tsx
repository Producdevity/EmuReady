...
function ManageSupportedDevicePlatforms(props: Props) {
  ...
  const groupedPlatforms = useMemo(() => {
    ...
    return SCOPE_ORDER.map((scope) => ({
      scope,
      platforms: groups.get(scope) ?? [],
    })).filter((group) => group.platforms.length > 0)
  }, [props.allPlatforms, searchQuery])

  const utils = api.useUtils()
  const updateMutation = api.devices.updateSupportedPlatforms.useMutation({
    onSuccess: async () => {
    ...
    onError: (error) => {
      ...
      console.error('Error updating device supported platforms:', error)
    },
  })
  ...
  const handleSaveChanges = () => {
    ...
    } satisfies RouterInput['devices']['updateSupportedPlatforms'])
  }
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Select every platform this device can run. The device&apos;s default platform is set in the
        next section and must be one of the supported platforms.
      </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {platforms.map((platform) => {
                ...
                return (
                  <label
                    key={platform.id}
                    htmlFor={`dev-platform-${platform.id}`}
...
export default ManageSupportedDevicePlatforms
